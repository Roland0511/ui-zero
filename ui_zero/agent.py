...
def take_action(adb: ADBTool, output: ActionOutput):
    if output.is_click_action():
        ...
        adb.drag(output.start_point_abs[0], output.start_point_abs[1], output.end_point_abs[0], output.end_point_abs[1], 500)
...
class AndroidAgent():
    ...
    def run(self, prompt: str, max_iters: int = 10,
        ...
        while max_iters > 0:
            ...
            # Run the model
            output = self.model.run(full_prompt, img_bytes, stream_resp_callback=stream_resp_callback, debug=debug)
