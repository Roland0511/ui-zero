...
class TestRunner:
    ...
    def run_step(self, step: str, 
                ...
                stream_resp_callback: Optional[Callable[[str, bool], None]] = None) -> ActionOutput:
        """
        执行单个测试步骤
...
def run_testcases(testcase_prompts: list, 
                 screenshot_callback: Optional[Callable[[bytes], None]] = None,
                 preaction_callback: Optional[Callable[[str, ActionOutput], None]] = None,
                 postaction_callback: Optional[Callable[[str, ActionOutput], None]] = None,
    ...
    if is_cli_mode:
        ...
        if debug:
            debug_history_key = 'debug_history_enabled' if include_history else 'debug_history_disabled'
...
def execute_single_step(step: str, agent: Optional[AndroidAgent] = None,
                       screenshot_callback: Optional[Callable[[bytes], None]] = None,
                       preaction_callback: Optional[Callable[[str, ActionOutput], None]] = None,
                       postaction_callback: Optional[Callable[[str, ActionOutput], None]] = None,
                       stream_resp_callback: Optional[Callable[[str, bool], None]] = None,
                       device_id: Optional[str] = None) -> ActionOutput:
    """执行单个测试步骤（用于GUI模式）"""
    if agent is None:
        adb_tool = ADBTool(device_id=device_id) if device_id else ADBTool()
        agent = AndroidAgent(adb=adb_tool)
...
def run_testcases_for_gui(testcase_prompts: list,
                         screenshot_callback: Optional[Callable[[bytes], None]] = None,
                         preaction_callback: Optional[Callable[[str, ActionOutput], None]] = None,
                         postaction_callback: Optional[Callable[[str, ActionOutput], None]] = None,
                         stream_resp_callback: Optional[Callable[[str, bool], None]] = None,
...
USEAGE EXAMPLES:
  ...
  %(prog)s --help                               # Show this help message
    ...
    group.add_argument(
        '--testcase', '-t',
        type=str,
        help=get_text('arg_testcase_help')
    )
    
    group.add_argument(
        '--command', '-c',
        action='append',
        help=get_text('arg_command_help')
    )
    
    parser.add_argument(
        '--version', '-v',
        action='version',
        version='UI-Zero v0.1.0'
    )
    
    parser.add_argument(
        '--no-history',
        action='store_true',
        help=get_text('arg_no_history_help')
    )
    
    parser.add_argument(
        '--debug', '-d',
        action='store_true',
        help=get_text('arg_debug_help')
    )
    
    parser.add_argument(
        '--device', '-D',
        type=str,
        help=get_text('arg_device_help')
    )
    
    parser.add_argument(
        '--list-devices',
        action='store_true',
        help=get_text('arg_list_devices_help')
    )
    ...
    if args.command:
        ...
        if os.path.exists(default_testcase_file):
            ...
            print(get_text('loaded_from_default', default_testcase_file, len(testcase_prompts)))
        else:
            # 没有找到可用的测试用例
    ...
    # 执行测试用例
    include_history = not args.no_history  # --no-history 为 True 时，include_history 为 False
    run_testcases(testcase_prompts, include_history=include_history, debug=args.debug, is_cli_mode=True, device_id=args.device)
